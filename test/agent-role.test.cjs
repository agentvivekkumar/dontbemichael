...
test('preferredAgentRole keeps a hire role over standby', () => {
  ...
  assert.equal(preferredAgentRole(undefined, undefined, true), 'office manager');
});
...
test('roleForHiveSpawn omits a transient roster caption', () => {
  ...
  assert.equal(
    roleForHiveSpawn({ description: 'office manager: runs the floor and brings you only the critical calls', isGod: true }),
    'office manager'
  );
});
